...
function bundleNodeRuntime() {
    ...
    const npmDirCandidates = [
        ...
        join(nodeDir, '..', 'libexec', 'lib', 'node_modules', 'npm'),
    ].map(candidate => resolve(candidate));
    const npmDir = npmDirCandidates.find(candidate => existsSync(candidate));

    if (!existsSync(nodeExecutable)) {
        throw new Error(`Node executable not found: ${nodeExecutable}`);
    }
    if (!npmDir) {
        throw new Error(`Bundled npm directory not found. Checked: ${npmDirCandidates.join(', ')}`);
    }
