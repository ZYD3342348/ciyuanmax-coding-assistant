...
const GITHUB_API_URL = 'https://api.github.com/repos/ZYD3342348/ciyuanmax-coding-assistant/releases/latest';
const REMOTE_CONFIG_URL = 'https://raw.githubusercontent.com/ZYD3342348/ciyuanmax-coding-assistant/main/remote-config.json';
...
function httpsGet(url: string, proxy?: string): Promise<{ statusCode: number; headers: http.IncomingHttpHeaders; body: string }> {
    return new Promise((resolve, reject) => {
        ...
        const options: https.RequestOptions = {
            ...
            method: 'GET',
            headers: {
                'User-Agent': `CiYuanMaxCodingAssistant/${app.getVersion()}`,
...
function testMirrorSpeed(mirrorBaseUrl: string, downloadUrl: string, timeoutMs: number = 5000): Promise<{ name: string; url: string; timeMs: number }> {
    return new Promise((resolve) => {
        ...
        const options: http.RequestOptions = {
            ...
            path: parsedUrl.pathname + parsedUrl.search,
            method: 'HEAD',
            headers: { 'User-Agent': `CiYuanMaxCodingAssistant/${app.getVersion()}` },
            timeout: timeoutMs,
        };
...
export class AppUpdater {
    ...
    async downloadUpdate(downloadUrl: string): Promise<DownloadResult> {
        try {
            ...
            const fileName = path.basename(new URL(downloadUrl).pathname) || 'CiYuanMaxCodingAssistant-Setup.exe';
