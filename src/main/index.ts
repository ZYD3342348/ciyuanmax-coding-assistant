...
function setupIPC(): void {
    ...
    ipcMain.handle('provision:create-keys', async () => {
        ...
        if (result.success) {
            ...
            if (result.claudeKey) {
                ...
                configStore.setBaseUrl('anthropic', 'https://llm.ciyuanmax.art');
            }
            if (result.codexKey) {
                configStore.setApiKey('openai', result.codexKey);
                configStore.setBaseUrl('openai', 'https://llm.ciyuanmax.art/v1');
            }
        }
        return result;
    });
    ...
    ipcMain.handle('clipboard:read-image', async () => {
        try {
            ...
            const dir = path.join(os.tmpdir(), 'ciyuanmax-coding-assistant-paste');
