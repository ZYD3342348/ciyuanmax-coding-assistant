...
const CIYUANMAX_DEFAULTS = {
    anthropicBaseUrl: 'https://llm.ciyuanmax.art',
    openaiBaseUrl: 'https://llm.ciyuanmax.art/v1',
...
function setupWelcomeScreen() {
    ...
    btnSave?.addEventListener('click', async () => {
        ...
        await api.config.setBaseUrl('anthropic', anthropicBaseUrl || CIYUANMAX_DEFAULTS.anthropicBaseUrl);
        await api.config.setBaseUrl('openai', openaiBaseUrl || CIYUANMAX_DEFAULTS.openaiBaseUrl);
        await api.config.setModel('anthropic', CIYUANMAX_DEFAULTS.anthropicModel);
        await api.config.setModel('openai', CIYUANMAX_DEFAULTS.openaiModel);
...
function prefillWelcomeDefaults() {
    const anthropicBaseUrlInput = /** @type {HTMLInputElement} */ ($('#baseurl-anthropic'));
    const openaiBaseUrlInput = /** @type {HTMLInputElement} */ ($('#baseurl-openai'));
    if (anthropicBaseUrlInput && !anthropicBaseUrlInput.value) {
        anthropicBaseUrlInput.value = CIYUANMAX_DEFAULTS.anthropicBaseUrl;
    }
    if (openaiBaseUrlInput && !openaiBaseUrlInput.value) {
        openaiBaseUrlInput.value = CIYUANMAX_DEFAULTS.openaiBaseUrl;
    }
}
